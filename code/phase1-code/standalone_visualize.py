#!/usr/bin/env python3
"""
Standalone EEG Visualization Generator
======================================

Generates visualizations from already-preprocessed EEG datasets.
Use this AFTER fixing labels to regenerate visualizations with correct labels.

Usage:
    python standalone_visualize.py --dataset TUAB
    python standalone_visualize.py --dataset TUEP
    python standalone_visualize.py --dataset TUEV
    python standalone_visualize.py --dataset all
...
class StandaloneEEGVisualizer:
    ...
    def generate_all(self, preprocessed_dir: Path, output_dir: Path):
        ...
        # RANDOM sample to find label distribution (not just first 1000)
        import random
        ...
        print("🔍 DEBUG: Labels in loaded sample_data:")
        for i, data in enumerate(sample_data):
            label = data['metadata'].get('label', 'unknown')
            print(f"  Sample {i+1}: {label}")
        print()
        
        # Load config from first segment
        config = sample_data[0].get('config', {})
        ...
        for plot_type in self.plot_types:
            try:
                ...
                if plot_type == 'raw_eeg_traces':
                    ...
                    self._plot_preprocessing_pipeline(config, output_dir)
                elif plot_type == 'quality_metrics':
                    self._plot_quality_metrics(stats, config, output_dir)
    ...
    def _compute_statistics(self, preprocessed_dir, segment_files):
        ...
        # RANDOM sample for label distribution (not just first 1000)
        import random
...
(Extrapolated to full dataset)
    ...
    def _plot_preprocessing_pipeline(self, config, output_dir):
        ...
        steps = [
            ...
            ("2. Channel Selection", f"Target: {preprocessing.get('n_channels', 20)} channels\n"
                                    f"TCP montage\nFirst-electrode selection"),
            ("3. Resample", f"Target: {preprocessing.get('target_sfreq', 200)} Hz\n"
                           f"Polyphase FIR"),
            ("4. Bandpass Filter", f"Range: {preprocessing.get('bandpass_filter', {}).get('low', 0.5)}-"
                                  f"{preprocessing.get('bandpass_filter', {}).get('high', 75)} Hz\n"
                                  f"Method: FIR zero-phase"),
            ("5. Notch Filter", f"Frequency: {preprocessing.get('notch_filter', {}).get('frequency', 60)} Hz\n"
                               f"Remove powerline noise"),
            ("6. Segmentation", f"Duration: {preprocessing.get('segment_duration', 10)}s\n"
                               f"Non-overlapping"),
            ("7. Normalization", f"Method: Global z-score\n"
                                f"Epsilon: {preprocessing.get('normalization', {}).get('epsilon', '1e-8')}"),
            ("8. Quality Control", f"Amplitude threshold\nVariance check\nFlat channel detection")
        ]
    ...
    def _plot_quality_metrics(self, stats, config, output_dir):
...
"""
        ...
        qc_params = config.get('preprocessing', {}).get('quality_control', {})
        if qc_params:
            summary_text += f"\nQC THRESHOLDS:\n{'-' * 40}\n"
            summary_text += f"Max Amplitude: {qc_params.get('max_amplitude_threshold', 'N/A')} µV\n"
            summary_text += f"Min Median STD: {qc_params.get('min_median_std', 'N/A')}\n"
            summary_text += f"Flat Threshold: {qc_params.get('flat_threshold', 'N/A')}\n"
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate visualizations from preprocessed EEG datasets',
...
Examples:
  ...
  # Visualize TUEP with 10 samples
  python standalone_visualize.py --dataset TUEP --max-samples 10

  # Visualize all datasets
  python standalone_visualize.py --dataset all

  # Custom paths
  python standalone_visualize.py --dataset TUAB --preprocessed-dir custom/path --output-dir custom/output
        """
    )
